...
class StableEmbeddingService:
    ...
    def _setup_providers(self):
        ...
        if self.openrouter_api_key or os.getenv("OPENROUTER_API_KEY"):
            ...
            logger.warning("⚠️ OpenRouter API key not configured - using mock only")
        
        # Fallback: Mock provider (always available)
        self._providers["mock"] = MockProvider(timeout=1.0)
        self._provider_order.append("mock")
        
        logger.info(f"🔧 Configured providers: {self._provider_order}")
    ...
    @staticmethod
    def create_for_demo(openrouter_api_key: Optional[str] = None) -> 'StableEmbeddingService':
        """
        Create service configured for demonstration.
        
        Args:
            openrouter_api_key: Optional OpenRouter API key
            
        Returns:
            Configured StableEmbeddingService
        """
        return StableEmbeddingService(
            openrouter_api_key=openrouter_api_key,
            redis_url=None,  # Disable Redis for demo
            cache_ttl=3600,  # 1 hour for demo
            batch_size=10,   # Small batches for demo
            embedding_timeout=15.0,  # Generous timeout
        )
...
async def quick_embed(text: str, openrouter_api_key: Optional[str] = None) -> EmbeddingResult:
    """Quick embedding for single text."""
    service = await create_embedding_service(openrouter_api_key)
    try:
...
if __name__ == "__main__":
    ...
    async def demo():
        """Demonstration of the stable embedding service."""
        print("🚀 Stable Embedding Service Demo")
        print("=" * 50)
        
        # Create service (demo mode - no Redis, no API key required)
        ...
        print(f"🔧 Providers: {health['provider_order']}")
        
        # Test single embedding
        print("\n🔤 Testing single embedding...")
        result = await service.embed_text("Привет, мир! Это тестовое предложение.")
        ...
        texts = [
            "Первый текст для проверки",
            "Второй текст для демонстрации",
            "Третий текст с другими словами"
        ]
